...
// ── Drink picker sync (Jul 18) ──────────────────────────────────────────────
// "Pair by what you have": a drink select at the top of main-menu that dims
// dishes it doesn't pair with. Generated HERE from canon (drinks.js + each
// dish's pairing ids) between DRINK_PICKER markers, so the index can never
// drift from the pairings — same contract as the pairings blocks above.
import { DRINKS } from '../src/drinks.js';
function drinkPickerBlock() {
  const index = {};   // drinkId -> [dish names]
  for (const d of DISHES) {
    for (const pr of (d.copy && d.copy.pairings) || []) {
      if (!pr.id) continue;
      (index[pr.id] = index[pr.id] || []).push(d.name);
    }
  }
  const ids = Object.keys(index).sort((a, b) => (DRINKS[a].label).localeCompare(DRINKS[b].label));
  const opts = ids.map(id =>
    `<option value="${id}">${DRINKS[id].label} (${index[id].length})</option>`).join('');
  return `<!-- DRINK_PICKER_START (generated by syncMainMenu — edit dishes.js, not this) -->
<div class="drink-picker">
  <div class="dp-head">Pair by what you have</div>
  <div class="dp-sub">Pick a drink; dishes it doesn't go with fade back.</div>
  <select id="drinkPick"><option value="">Show everything</option>${opts}</select>
</div>
<script>
(function(){
  var IDX = ${JSON.stringify(index)};
  var sel = document.getElementById('drinkPick');
  if (!sel) return;
  sel.addEventListener('change', function(){
    var names = sel.value ? (IDX[sel.value] || []) : null;
    var cards = document.querySelectorAll('.dish');
    for (var i = 0; i < cards.length; i++) {
      var nameEl = cards[i].querySelector('.dish-name');
      var name = nameEl ? nameEl.textContent.trim() : '';
      cards[i].style.opacity = (!names || names.indexOf(name) >= 0) ? '' : '0.25';
    }
  });
})();
</script>
...
{
  const want = drinkPickerBlock();
  const re = /<!-- DRINK_PICKER_START[\s\S]*?<!-- DRINK_PICKER_END -->/;
  const m = html.match(re);
  if (!m || m[0] !== want) {
    drift++;
    console.log(`  drink picker: ${m ? 'stale' : 'MISSING'}`);
    if (write) {
      if (m) html = html.replace(re, want);
      else {
        // First install: directly after the opening of the content, before the
        // first section head.
        const anchor = html.indexOf('<div class="section-head">');
        html = html.slice(0, anchor) + want + '\n\n' + html.slice(anchor);
      }
      patched++;
    }
  }
