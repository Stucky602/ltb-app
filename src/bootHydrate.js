...
import { normalizeAccommodations } from './accommodation.js';
import {
...
export async function hydrateFromStorage(deps) {
  const {
    ...
    setAuditLog, setNotice, setRealDataEpoch, setRowanLog, setDishRankings, setVisualCues, setCustomerFlags, setPractices, setCaptureInbox, setLabelVersions, setWalkAnswers, setTerms, setAnatomy, setDerivatives, setRowanQuestions, setClarifications, setNotesRowan, setDecisionLedger, setRowanBoards, setRowanRoles, setHouseholdMemories, setPassportCabinets, setAccommodations,
  ...
  const [loadedOrders, loadedChecks, loadedShopping, loadedWeek, loadedDeliverChecks, loadedDishNotes, loadedDishFeedback, loadedPipelineJournal, loadedJournal, loadedLastSeenWeek, loadedContainerCfg, loadedLedger, loadedCopiesNote, loadedArchiveHistory, loadedEpoch, loadedRowan, loadedRankings, loadedCues, loadedFlags, loadedPractices, loadedCapture, loadedLabels, loadedWalks, loadedTerms, loadedAnatomy, loadedDerivatives, loadedQuestions, loadedClarifications, loadedNotes, loadedDecisions, loadedBoards, loadedRoles, loadedMemories, loadedCabinets, loadedAccommodations] = await Promise.all([
    ...
    loadJSON(ACCOMMODATIONS_KEY, null),
  ]);
  if (!isMounted()) return;
  ...
  setAccommodations(normalizeAccommodations(loadedAccommodations));
