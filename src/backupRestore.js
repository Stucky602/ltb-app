...
export function buildBackupPayload(state) {
  return {
    ...
    dishRankings: state.dishRankings,
...
export async function applyBackupPayload(payload, deps) {
  ...
  if (Array.isArray(payload.dishRankings)) {
    setDishRankings(payload.dishRankings);
    await saveJSON(DISH_RANKING_KEY, payload.dishRankings);
  }
  ...
  // Notes to Rowan exist nowhere else and cannot be reconstructed by anyone.
  if (payload.notesRowan && typeof payload.notesRowan === 'object') {
    setNotesRowan(payload.notesRowan);
    await saveJSON(NOTES_ROWAN_KEY, payload.notesRowan);
  }
  ...
  // Rulings he made once and should never have to make twice.
  if (payload.accommodations && typeof payload.accommodations === 'object') {
    setAccommodations(payload.accommodations);
    await saveJSON(ACCOMMODATIONS_KEY, payload.accommodations);
  }
