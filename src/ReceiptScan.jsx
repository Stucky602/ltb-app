...
import { buildReviewPlan, defaultAccept, normalizeUnit, convertPerUnit, parsePastedReceipt, learnFromAcceptance, learnFromIgnores, learnStoreFact, packShiftAlarm, reconcileReceipt, priceDriftReport, derivePerUnit, mergeExtractions, perUnitFromUserWeight, WEIGHT_ENTRY_UNITS, perUnitFromUserCount, shouldAskCount, autoApplyAllowed } from '../receiptMatch.js';
    ...
    <div>
      ...
      {matched.length > 0 && (
        <Section title="Matched — confirm each">
          {[...matched].sort((a, b) => (TIER_ORDER[a.r.tier] ?? 1) - (TIER_ORDER[b.r.tier] ?? 1)).map(({ r, i }) => <MatchedRow key={i} r={r} idx={i} patchRow={patchRow} onOpenPicker={onOpenPicker} costHistory={costHistory} seed={seed} aliases={localAliases} />)}
        </Section>
      )}
...
function MatchedRow({ r, idx, patchRow, onOpenPicker, costHistory, seed, aliases }) {
  ...
  // Only offer the count prompt when the app actually GUESSED. shouldAskCount
  // checks the ingredient is piece-priced AND the basis is the line-total
  // fallback AND no PACK_OVERRIDE or learned packQty already answered it.
  // Asking a question whose answer is on file is how prompts get dismissed
  // reflexively, which costs the ones that matter.
  const askCount = !!ing && shouldAskCount({
    ingUnit: ing.unit,
    basis: r.basis,
    ingredientId: r.ingredientId,
    // Rows carry `norm` (the normalized receipt string), not the alias record
    // itself, so look it up. Skipping this would silently disable the
    // learned-packQty guard and re-ask a question already answered.
    alias: (aliases || {})[r.norm],
  });
  ...
  return (
    <div style={{ ...S.row, ...(r.accept ? S.rowOn : {}) }}>
      ...
      {/* Jul 15: the drift box above used to be nested INSIDE the sentence
          below, between "line total ($2.25" and "), not a per-ear price". It
          compiled fine and rendered as a sentence cut in half by an alert box.
          Untangled; the two are separate siblings now. */}
      {mismatchRisk && (
        <div style={S.warn}>
      ...
      {/* The escape hatch for the corn case. mismatchRisk says "the app fell
          back to the line total"; askCount says that fallback is specifically
          a per-piece guess Kevin can correct with one number he knows. */}
      {askCount && (
        <CountEntry
          lineTotal={r.line.line_total}
          ingUnit={ing.unit}
          onApply={(pu) => patchRow(idx, { acceptedPerUnit: pu, basis: 'user_count', accept: true })}
        />
      )}
...
// ── C.6: count entry (Jul 15) ───────────────────────────────────────────────
// The twin of WeightEntry, for ingredients priced by the piece. Kevin types
// how many were in the bag; the app derives the per-unit price.
//
// The bug it closes: "24 FRESH CORN ... 2.25" is a line TOTAL with no count
// (the 24 is a PLU). derivePerUnit's last resort treats a bare total as the
// per-unit price, so the app decided corn was $2.25/ear. He bought nine.
// Weight-priced items already had an escape hatch; count-priced ones had
// none, because you cannot weigh your way out of "how many lemons was that."
//
// Same contract as WeightEntry deliberately — one input, math shown inline,
// one "Use" button. A third distinct shape for a fourth kind of arithmetic is
// how a scan screen becomes something you skim past instead of read.
function CountEntry({ lineTotal, ingUnit, onApply }) {
  const [count, setCount] = useState('');
  const derived = perUnitFromUserCount({ total: lineTotal, count, ingUnit });
  const n = parseFloat(count);
  const num = { ...S.priceInput, flex: 'none', width: 60, padding: '7px 8px', textAlign: 'center' };
  const lbl = { color: '#9aa5a0', fontSize: 12 };
  // Plural only where it reads naturally. "9 ears" and "6 eaches" are not the
  // same quality of English, and this line is the whole point of the prompt.
  const unitWord = (qty) => {
    if (ingUnit === 'each') return qty === 1 ? 'item' : 'items';
    return qty === 1 ? ingUnit : `${ingUnit}s`;
  };
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ ...lbl, marginBottom: 4 }}>
        The receipt shows a total but not a count. How many did you buy?
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        <input
          style={num} type="number" inputMode="decimal" step="1" min="0" placeholder="9"
          value={count} onChange={e => setCount(e.target.value)}
        />
        <span style={lbl}>{unitWord(n)}</span>
      </div>
      {derived && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
          {/* Show the arithmetic. A derivation Kevin can't check is a
              derivation he has to trust, and this whole prompt exists
              because something untrustworthy got trusted. */}
          <span style={{ fontSize: 12, color: '#5DCAA5' }}>
            {currency(lineTotal)} ÷ {derived.count} {unitWord(derived.count)} = {currency(derived.perUnit)}/{ingUnit}
          </span>
          <button style={S.flatBtn} onClick={() => onApply(derived.perUnit)}>Use count</button>
        </div>
      )}
    </div>
  );
